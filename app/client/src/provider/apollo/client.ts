 ...
 *
 * A plain chain, not an `ApolloLink.split`: the schema has no subscriptions.
 * `scanProgress` was its only one, and it went with the library scan, taking
 * the `SSELink`/graphql-sse transport that carried it — every operation the
 * client sends today is an HTTP query or mutation.
 *
