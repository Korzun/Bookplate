 ...
 * — mirrors REST's `POST /api/books/:id/link` (`routes/ui.ts`, removed in
 * `e67b4ad9`), whose body was `{ documentId: string }`. The `Book` global ID IS
 * the input's `id` field — no separate `userId`/`bookId` pair. Same shape as
 ...
 * → 400 `{ error: 'documentId is required' }` (`routes/ui.ts`, removed in
 * `e67b4ad9`). The TRIMMED value is what gets passed to `linkDocument` below,
 * exactly like REST passed `documentId.trim()` to it. `bookId` has no zod rule
 ...
 * (`InvalidInputError` for an empty `bookId` is unreachable now the same way it
 * became unreachable for `bookDelete`'s identical field in task 2: an id that
 * doesn't parse is the `parsed === null` early return below, not a zod issue).
 */
...
/**
 * Mirrored REST's `POST /api/books/:id/link` (`routes/ui.ts`, removed in
 * `e67b4ad9`). Input is the `Book` global ID alone plus `documentId` (design
 * doc's 10-mutation input collapse), decoded with the same
 ...
 *
 * `linkDocument` (`services/book-lineage.ts`) throws exactly three of
