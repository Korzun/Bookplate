 ...
 * `book-hash-collision-error/model.ts` carries the service's own error class —
...
export const model = builder.objectRef<ScanStatusShape>('ScanStatus').implement({
  ...
  fields: (t) => ({
    ...
    // underlying value, `parent.job.jobId` (the scan-job registry's own field
    ...
    // optional independent of `status` in `ScanJob` itself, but the registry
    // never sets one without the other (`reduceScanJob`'s `'complete'`
