...
describe('refresh tokens', () => {
  it('creates a token that can be consumed once', async () => {
    const token = await createRefreshToken(prisma, { username: 'alice', userId: aliceId });
    expect(typeof token).toBe('string');
    expect(token.length).toBeGreaterThanOrEqual(43); // 32 bytes base64url

    const identity = await consumeRefreshToken(prisma, token);
    expect(identity).toEqual({ username: 'alice', userId: aliceId });

    // rotation: a second consume of the same token fails
    expect(await consumeRefreshToken(prisma, token)).toBeNull();
  });

  it('supports a null userId (config admin)', async () => {
    const token = await createRefreshToken(prisma, { username: 'admin', userId: null });
    expect(await consumeRefreshToken(prisma, token)).toEqual({ username: 'admin', userId: null });
  });

  it('rejects unknown tokens', async () => {
    expect(await consumeRefreshToken(prisma, 'not-a-real-token')).toBeNull();
  });
  ...
  it('sets expiry ~30 days out', async () => {
    await createRefreshToken(prisma, { username: 'alice', userId: aliceId });
  ...
  it("revokeAllForUsername deletes only that user's tokens", async () => {
    const a = await createRefreshToken(prisma, { username: 'alice', userId: aliceId });
    const b = await createRefreshToken(prisma, { username: 'bob', userId: bobId });
    await revokeAllForUsername(prisma, 'alice');
    expect(await consumeRefreshToken(prisma, a)).toBeNull();
    expect(await consumeRefreshToken(prisma, b)).toEqual({ username: 'bob', userId: bobId });
  });

  it('deleteExpired sweeps only expired rows', async () => {
    const live = await createRefreshToken(prisma, { username: 'alice', userId: aliceId });
    await createRefreshToken(prisma, { username: 'bob', userId: bobId });
